      ...
      <SafeAreaProvider>
        <ThemeProvider value={NAV_THEME}>
          ...
          >
            ...
            {/*
              스와이프로 나가도 고치던 값이 조용히 사라지지 않는다 — 뒤로가기 버튼과
              같은 물음을 edit.tsx의 usePreventRemove가 붙잡아 띄운다. 제스처를 껐던
              이유가 그 물음이 없었던 것뿐이라, 물음이 생긴 지금은 켠다.

              ⚠️ iOS 26에서는 화면 어디서 밀어도 뒤로 간다(가장자리 전용이 아니다).
              이름칸 안에서 가로로 손가락을 끌면 글자 사이를 짚는 대신 화면이 넘어간다 —
              시뮬레이터(iOS 26.5)에서 실제로 그랬다. 넘어가도 고친 것이 있으면 위의
              물음이 서므로 값을 잃지는 않는다. 좁혀 보려 했지만 길이 없었다:
              · fullScreenGestureEnabled:false 로 끄면 가장자리 스와이프까지 같이 죽는다
                (iOS 26은 둘이 같은 인식기다). 왼쪽 끝에서 밀어도 아무 일이 없었다
              · gestureResponseDistance 로 구간을 좁히면(end 16·24·60 모두) 제스처가
                거의 서지 않았다 — 인식 시점의 손가락 위치로 재는 값이라, 미는 동안
                이미 구간을 벗어난다. 8번 중 1번 걸렸다
            ...
            */}
            ...
            {/*
              편집 화면에 스와이프를 되살리면서 여기도 함께 열어 보고 되돌렸다.
              iOS 26은 화면 어디서 밀어도 뒤로 가므로, 알림음 볼륨을 **올리는**
              손짓(좌→우)이 그대로 뒤로가기가 된다 — 슬라이더를 오른쪽으로 끌자
              설정이 닫히고 홈이 나왔다(시뮬레이터 iOS 26.5에서 확인).

              ⚠️ 구간을 좁혀 피하려던 길은 막혀 있다. 예전 커밋의 start:16이 듣지
              않았던 것은 값이 틀려서였고(start·end는 "가장자리에서 몇 pt"가 아니라
              제스처를 받을 구간의 왼쪽·오른쪽 경계다 — RNSScreenStack.mm
              isInGestureResponseDistance), 바로잡아 end:16·24·60으로 재 봐도 이번엔
              제스처 자체가 서지 않았다. 인식이 끝난 시점의 손가락 위치로 재기 때문에
              미는 동안 이미 구간 밖이다. 8번 중 1번 걸렸다.

              그래서 여기는 그대로 둔다 — 나가는 길은 왼쪽 위 닫기 버튼이다.
              슬라이더를 24pt 안쪽으로 물리는 것은 답이 아니다. 왼쪽 끝까지 끌어야
              0이 되는 물건이라 양보할 수가 없다.
            */}
            <Stack.Screen
              name="settings"
