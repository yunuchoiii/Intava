...
### 자동 넘김 (index.html 아래쪽 JS)

2초마다 한 장씩 자동으로 넘어간다. 규칙 셋:

- **자동이 손을 이기면 안 된다.** 손가락이 닿아 있는 동안은 쉬고, 어떤 스크롤이든
  (손이든 자동이든) 완전히 멎은 뒤 2초에 다음 장이다 — 예약은 scroll 리스너의
  `plan()`이 하므로 별도 체인이 없다
- **모션 줄이기에서는 아예 돌리지 않는다**
- **탭이 뒤로 가 있거나 덱이 화면 밖이면 그 판을 건너뛴다** — 돌아오는 순간
  몰아서 넘어가는 것을 막는다

⚠️ 클로드 코드의 Browser 패널에서는 페이지가 스스로를 백그라운드로 여겨
(`document.hidden` 참) 자동 넘김이 쉬는 것처럼 보인다 — 버그가 아니다.
판정은 실제 크롬에서 하거나, 패널에서는 `document.hidden`을 가짜로 풀고 본다.
...
기록 화면은 목업 도구로 따로 뽑은 게 없어서, 히어로 목업의 **프레임만 재활용해** 끼웠다.
목업(1419×2796)의 화면 자리는 사방 120px 안쪽이고 모서리 반지름은 178px 이다.
그 둥근 사각형 안쪽 픽셀만 새 스크린샷(1179×2556 으로 리사이즈)으로 갈아 끼우면
프레임의 모서리 blend 와 은색 레일이 그대로 남는다.
같은 화면을 다시 뽑을 땐 **시뮬레이터 원본(1206×2622)** 이 있어야 한다 — 배포된 620px
webp 를 늘리면 뭉갠 티가 난다.
...
을 걸어 둔 상자였다. 그 상자가 안에 있는 폰의 `box-shadow` 와 글로우 자신까지 잘라서,
그림자가 직선으로 끊기고 사각형 경계가 드러났다. **여백을 줘서 그림자를 상자 안에
넣는 것으로는 부족하다** — 글로우의 잘린 자리는 그대로 남았다.

답은 **자르지 않는 것**이다. 글로우를 제 상자 안(`inset:0`)에서 72%에 완전히 투명해지게
만들면 삐져나갈 게 없어 `overflow` 를 걸 이유가 사라진다.
히어로 카드 리스트의 `padding-bottom` 도 같은 이유(자르는 상자 + 그림자)로 넉넉히 잡은 것이다.

그리고 CSS 로 그린 베젤은 배경(#17171C)보다 어두워서 검은 그림자가 배경에 묻히고
테두리 선만 남는다 — 폰이 아니라 잘라 붙인 판때기로 보인다. **기록 섹션도 히어로와
같은 실사 목업으로 바꿨다**(아래 "기록 섹션 목업 만드는 법").
...
현재 첫 화면이 받는 것이 313KB(그중 카드 다섯 장이 214KB)다.
